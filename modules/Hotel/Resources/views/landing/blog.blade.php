    ...
    <div class="lg:col-span-2 space-y-8">
      @forelse($posts as $post)
        ...
        <article class="hb-card overflow-hidden group transition hover:shadow-panel">
          @if($postImage)
            <a href="{{ url('blog/'.$post->slug) }}" class="relative block aspect-[16/8] bg-ink-100 overflow-hidden">
          ...
          <div class="p-7">
            ...
            <h2 class="font-display text-2xl font-bold text-ink-900 leading-snug mb-3"><a href="{{ url('blog/'.$post->slug) }}" class="hover:text-brand transition">{{ $post->title }}</a></h2>
            <p class="text-[15px] text-ink-500 leading-relaxed mb-5">{{ $excerpt }}</p>
            <a href="{{ url('blog/'.$post->slug) }}" class="hb-btn hb-btn-ghost">Leer más <i class="fa fa-angle-right"></i></a>
          </div>
        </article>
      @empty
        ...
        <div class="hb-card p-6">
          ...
          <ul class="space-y-4">
            @foreach($posts->take(4) as $recent)
              ...
              @endphp
              <li>
                <a href="{{ url('blog/'.$recent->slug) }}" class="flex gap-3 group">
