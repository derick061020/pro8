...
class HotelRentController extends Controller
{
  ...
  public function extendTime(Request $request, $rentId)
  {
    try {
      ...
      // La tarifa vigente del alquiler pasa a ser la de esta extensión: si el
      // recepcionista extiende con otro precio, ese es el precio que debe
      // mostrarse en la habitación y proponerse en la siguiente extensión.
      // Antes rental_price conservaba la tarifa del check-in y la extensión
      // siguiente volvía a arrancar con el precio antiguo.
      if ($unitPrice > 0) {
        $rent->rental_price = $unitPrice;
      }
  ...
  public function finalizeRent($rentId)
  {
    ...
    public function changeRoom($id, Request $request)
    {
        ...
        try {
            ...
            if ($openHabItems->isEmpty()) {
                ...
                // así que no hay nada "abierto" que cerrar/dividir: las noches
                // restantes YA fueron cobradas en el comprobante a la tarifa de la
                // habitación anterior y no se pueden recortar.
                //
                // Por eso aquí NO se vuelve a cobrar la estadía restante (eso era
                // un doble cobro: con la misma tarifa aparecía una deuda igual al
                // valor de las noches que faltaban, aunque el huésped ya las
                // hubiera pagado). Solo se cobra la DIFERENCIA de tarifa:
                //
                //   - noches ya cubiertas por el comprobante → (nueva − antigua)
                //     y únicamente si la nueva es más cara. Si la nueva es igual
                //     o más barata no se genera ningún cargo (una devolución
                //     sobre un comprobante emitido exige nota de crédito).
                //   - noches NO cubiertas por ningún comprobante (caso raro:
                //     comprobante parcial) → tarifa nueva completa.
                $allHab  = $rent->items()->where('type', 'HAB')->orderBy('id')->get();
                $lastHab = $allHab->last();

                // Tarifa realmente facturada de la habitación anterior (del item,
                // no de rent->rental_price, que puede estar desactualizado).
                $oldUnitPrice = $lastHab ? $resolveUnit($lastHab) : 0.0;
                if ($oldUnitPrice <= 0) {
                    $oldUnitPrice = (float) $rent->rental_price;
                }

                // Noches cubiertas por los comprobantes ya emitidos.
                $invoicedUnits = (int) $allHab->sum(function (HotelRentItem $it) use ($resolveQuantity) {
                    return $resolveQuantity($it);
                });

                $totalDuration = max(1, (int) ($rent->duration ?? 0), $invoicedUnits);
                ...
                // De las noches restantes, cuántas están cubiertas por el
                // comprobante (las facturadas se imputan desde el ingreso).
                $coveredRemaining   = max(0, min($remaining, $invoicedUnits - $consumed));
                $uncoveredRemaining = max(0, $remaining - $coveredRemaining);

                // Diferencia de tarifa por noche; solo se cobra si es a favor
                // del hotel (upgrade). Un downgrade no genera nota de crédito
                // automática.
                $unitDifference = round($newUnitPrice - $oldUnitPrice, 4);
                $chargeableDiff = $unitDifference > 0 ? $unitDifference : 0.0;
                ...
                // Cargo por las noches que NO cubre ningún comprobante (tarifa
                // nueva completa) + cargo por la diferencia de tarifa en las que
                // sí lo están.
                $fullTotal = round($newUnitPrice * $uncoveredRemaining, 4);
                $diffTotal = round($chargeableDiff * $coveredRemaining, 4);
                $newTotal  = round($fullTotal + $diffTotal, 4);

                $createdItems = [];

                if ($uncoveredRemaining > 0 && $fullTotal > 0) {
                    ...
                    $createdItems[] = HotelRentItem::create([
                        'type'                => 'HAB',
                        'hotel_rent_id'       => $rent->id,
                        'item_id'             => $newRoom->item_id,
                        'item'                => $this->rewriteHotelItemForRoom(
                            $baseJsonForNew,
                            $newItemRecord,
                            $newRoom,
                            $fullDescription,
                ...
                if ($coveredRemaining > 0 && $diffTotal > 0) {
                    $diffDescription = sprintf(
                        'Diferencia de tarifa por cambio a %s - %d %s (%s → %s)',
                        $newRoom->name,
                        $coveredRemaining,
                        $unitLabel,
                        $changeAt->format('d/m/Y H:i'),
                    ...
                    $createdItems[] = HotelRentItem::create([
                        ...
                        'payment_status'      => 'DEBT',
                        'hotel_rent_order_id' => null,
                        'quantity'            => $coveredRemaining,
                        'unit_price'          => $chargeableDiff,
                        'total'               => $diffTotal,
                        'description'         => $diffDescription,
                    ]);
                }

                $newItem = count($createdItems) ? $createdItems[count($createdItems) - 1] : null;
                ...
                if ($newTotal > 0) {
                    $resultMessage = 'Habitación cambiada. Se cobró solo la diferencia de tarifa de las noches restantes (S/ '
                        . number_format($newTotal, 2) . ').';
                } elseif ($unitDifference < 0) {
                    $resultMessage = 'Habitación cambiada. La nueva tarifa es menor y la estadía ya está facturada: no se generó deuda; '
                        . 'si corresponde devolver la diferencia, emita una nota de crédito.';
                } else {
                    $resultMessage = 'Habitación cambiada. La tarifa es la misma y la estadía ya está facturada: no se generó ninguna deuda.';
                }
                ...
                HotelRentChange::create([
                    ...
                    'new_values'       => [
                        ...
                        'item_ids'            => collect($createdItems)->pluck('id')->all(),
                        'consumed'            => $consumed,
                        'remaining'           => $remaining,
                        'covered_remaining'   => $coveredRemaining,
                        'uncovered_remaining' => $uncoveredRemaining,
    ...
    public function extendStay($id, Request $request)
    {
        ...
        try {
            ...
            // El precio de la última extensión pasa a ser la tarifa vigente del
            // alquiler (mismo criterio que extendTime).
            if ($unitPrice > 0 && (float) $rent->rental_price !== $unitPrice) {
                $rent->rental_price = $unitPrice;
                $rent->save();
            }
        ...
        try {
            ...
            if ($isEditing) {
                ...
                if ($isRefund) {
                    ...
                    // PAGO NORMAL.
                    //
                    // Mora (arrears): el checkout la mantiene solo en pantalla
                    // hasta finalizar el alquiler, así que si llega en el pago se
                    // persiste ANTES de calcular la deuda. Si no, el backend
                    // cobraba de menos y devolvía la mora como vuelto.
                    $requestArrears = $request->input('arrears');
                    if (is_numeric($requestArrears) && (int) $requestArrears !== (int) ($rent->arrears ?? 0)) {
                        $rent->arrears = (int) $requestArrears;
                        $rent->save();
                    }

                    // La deuda REAL siempre se calcula con la fórmula
                    // Σ items no-PAY − Σ pagos + mora (calculateRentDebt), NUNCA
                    // con `payment_status`: un item puede estar en PENDING
                    // (habitación rentada sin pagar), en PAID (se emitió el
                    // comprobante sin cobrar) o la deuda puede venir de la mora,
                    // que no cuelga de ningún item. Antes el pago solo se repartía
                    // entre los items en estado DEBT, así que todo lo que no
                    // calzaba ahí se devolvía como VUELTO: se cobraba 0 (o de
                    // menos) y se entregaba de vuelto el monto pagado.
                    $realDebt = round($this->calculateRentDebt($rent), 2);

                    // Sin deuda real -> el pago es un ADELANTO/crédito que queda
                    // disponible para futuros cargos (no genera vuelto).
                    if ($realDebt <= 0.009) {
                    ...
                    // Hay deuda: se cobra como máximo la deuda real y solo el
                    // excedente es VUELTO (el `change` nunca reduce la deuda ni
                    // entra a caja).
                    $appliedToDebt = round(min($amount, $realDebt), 2);
                    $change        = round($amount - $appliedToDebt, 2);

                    // Items cobrables del alquiler (habitación, extensiones y
                    // productos). Se recorren todos, sin filtrar por
                    // `payment_status`, y el saldo de cada uno sale de sus propios
                    // pagos.
                    $chargeableItems = HotelRentItem::where('hotel_rent_id', $rent->id)
                        ->where('type', '!=', 'PAY')
                        ->orderBy('id', 'asc')
                        ->get();
                    ...
                    $remainingAmount = $appliedToDebt;
                    $paymentItems    = [];
                    $lastPayment     = null;

                    foreach ($chargeableItems as $rentItem) {
                        if ($remainingAmount <= 0.009) break;

                        $itemData  = $rentItem->item;
                        $itemTotal = (is_object($itemData) && isset($itemData->total)) ? floatval($itemData->total) : 0;
                        if ($itemTotal <= 0) continue;

                        $totalPaid = floatval(
                            HotelRentItemPayment::where('hotel_rent_item_id', $rentItem->id)->sum('payment')
                        );

                        $remainingDebt = round($itemTotal - $totalPaid, 2);
                        if ($remainingDebt <= 0) {
                            // Item ya cubierto: se aprovecha para corregir su
                            // estado si había quedado como deuda.
                            if ($rentItem->payment_status !== 'PAID') {
                                $rentItem->payment_status = 'PAID';
                                $rentItem->save();
                            }
                            continue;
                        }

                        $paymentAmount = round(min($remainingAmount, $remainingDebt), 2);

                        $payment = new HotelRentItemPayment();
                        $payment->hotel_rent_item_id = $rentItem->id;
                        ...
                        // Actualizar estado del item si quedó completamente pagado.
                        $newTotalPaid = round($totalPaid + $paymentAmount, 2);
                        if ($newTotalPaid >= $itemTotal) {
                            $rentItem->payment_status = 'PAID';
                            $rentItem->save();
                        }

                        $remainingAmount = round($remainingAmount - $paymentAmount, 2);
                        ...
                        $paymentItems[] = [
                            ...
                            'remaining_debt' => max(0, round($itemTotal - $newTotalPaid, 2)),
                        ];
                    }

                    // Sobrante de lo aplicado a la deuda: corresponde a conceptos
                    // que no cuelgan de un item con saldo (típicamente la MORA).
                    // Se asocia al último item facturable para que la deuda real
                    // quede saldada y el monto entre a caja.
                    if ($remainingAmount > 0.009) {
                        $targetItem = $chargeableItems->last();

                        $payment = new HotelRentItemPayment();
                        $payment->hotel_rent_item_id = $targetItem->id;
                        $payment->payment = $remainingAmount;
                        $payment->payment_method_type_id = $paymentMethodTypeId;
                        $payment->reference = $reference;
                        $payment->change = 0;
                        $payment->date_of_payment = \Carbon\Carbon::now()->format('Y-m-d H:i:s');
                    ...
                    // El vuelto se guarda sobre el último pago creado: aparece en
                    // el historial y nunca se computa como ingreso ni como abono.
                    if ($change > 0 && $lastPayment) {
                        $lastPayment->change = $change;
                        $lastPayment->save();
                    }
                    ...
                    return response()->json([
                        ...
                        'change'         => $change,
                        'debt_before'    => $realDebt,
                        'remaining_debt' => round($realDebt - $appliedToDebt, 2),
                    ], 200);
                }
            }
            
            // Solo llega aquí la EDICIÓN de un pago existente: los pagos nuevos,
            // los adelantos y las devoluciones responden en su propia rama.
            DB::connection('tenant')->commit();
