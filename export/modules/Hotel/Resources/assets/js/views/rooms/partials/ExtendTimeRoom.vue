    ...
    >
      <form autocomplete="off" @submit.prevent="onSubmit">
        <div class="form-body">
          ...
          <!-- Primera Fila: Cantidad noches, Fecha salida, Hora salida -->
          <div class="row">
...
export default {
  ...
  computed: {
    extensionTotal() {
      // Calcular solo los días adicionales (form.duration ya son días adicionales)
      const total = (parseFloat(this.form.price_per_day) || 0) * Math.max(0, this.form.duration || 0);
      console.log('Extension Total calculado:', total);
      return total;
    },
    totalConsumption() {
  ...
  methods: {
    ...
    updateDuration() {
      // Calcular nueva fecha de salida sumando días adicionales a la fecha de salida actual
      const currentOutputDate = moment(this.room.rent.output_date, 'YYYY-MM-DD');
      const additionalDays = this.form.duration || 0;
      
      // IMPORTANTE: Usar clone() para no mutar la fecha original
      const newDate = currentOutputDate.clone().add(additionalDays, 'days');
      
      // Guardar en formato YYYY-MM-DD para la base de datos
      this.form.output_date = newDate.format('YYYY-MM-DD');
      
      console.log('Fecha salida actual:', currentOutputDate.format('YYYY-MM-DD'));
      console.log('Días adicionales:', additionalDays);
      console.log('Nueva fecha salida (formato BD):', this.form.output_date);
