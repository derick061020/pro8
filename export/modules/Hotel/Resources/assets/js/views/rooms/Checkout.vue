        ...
        <div class="card mb-0 tab-content-default row-new">
            <template v-if="canMakePayment">
                <div class="card-body invoice p-3">
                    ...
                    <div class="row card-body shadow-none mb-2 border-0" v-if="totalDebt > 0">
                            ...
                            <div class="timeline-content">
                                ...
                                <div class="change-details">
                                    ...
                                    <div v-if="change.change_type === 'EXTENSION'" class="change-detail">
                                        <strong>Extensión:</strong> {{ change.days }} días
                                        <br>
                                    ...
                                    <div v-if="change.change_type === 'ROOM_CHANGE'" class="change-detail">
                                        ...
                                        {{ change.old_values.room_name }} 
                                        <i class="fa fa-arrow-right"></i> 
                                        {{ change.new_values.room_name }}
                                    </div>
...
export default {
    ...
    computed: {
        ...
        rentPaidItems()
        {
            // Usar currentRent.items que se actualiza desde el servidor
            return (this.currentRent.items || []).filter(it => it.type === 'PRO' && it.payment_status === 'PAID')
        },
        rentDebtItems()
        {
            // Usar currentRent.items que se actualiza desde el servidor
            return (this.currentRent.items || []).filter(it => it.type === 'PRO' && it.payment_status === 'DEBT')
        },
        roomItems()
        {
            // Todos los items de tipo habitación (uno por tramo tras un cambio de habitación)
            const source = (this.rentItems && this.rentItems.length)
                ? this.rentItems
                : (this.currentRent.items || []);
            return source
                .filter(it => it.type === 'HAB')
                .slice()
    ...
    async mounted() {
        ...
        this.document.items = await this.currentRent.items
            ...
            .map(i => {
                ...
                const row = calculateRowItem(i.item, "PEN", 3, this.percentage_igv);
                row._rent_item_id = i.id;
                row._invoiced = !!(i.sale_note_id || i.document_id);
                row._document = i.document || null;
                console.log('document item built', { _rent_item_id: row._rent_item_id, _invoiced: row._invoiced, type: i.type });
                return row;
            });
    ...
    methods: {
        ...
        getRoomItemQuantity(r) {
            if (!r) return 0;
            const col = parseFloat(r.quantity);
            if (col > 0) return col;
            return parseFloat(r.item && r.item.quantity) || 0;
        },
        getRoomItemTotal(r) {
            if (!r) return 0;
            const col = parseFloat(r.total);
            if (col > 0) return col;
            return parseFloat(r.item && r.item.total) || 0;
        },
        ...
        async onGenerateInvoice() {
            ...
            try {
                ...
                console.log('pendingItems _rent_item_id values:', pendingItems.map(it => it._rent_item_id));

                const markPayload = {
                    sale_note_id: isSaleNote ? this.documentNewId : null,
                    document_id: isSaleNote ? null : this.documentNewId,
                    item_ids: pendingItems.map(it => it._rent_item_id).filter(Boolean),
                };
            ...
            try {
                ...
                if (Array.isArray(this.document.items)) {
                    this.document.items.forEach(docItem => {
                        const fresh = freshItems.find(r => r.id === docItem._rent_item_id);
                        if (fresh) {
                            this.$set(docItem, '_invoiced', !!(fresh.sale_note_id || fresh.document_id));
                            this.$set(docItem, '_document', fresh.document || null);
                        }
                    });
                }
            } catch (e) {
        ...
        printInvoice(invoice) {
            const url = invoice.type === 'sale_note'
                ...
                : `/documents/print/${invoice.external_id}/a4`;
        ...
        async recordRoomChange(changeType, oldValues, newValues, notes = null, priceDifference = 0) {
            try {
                ...
                // Recargar historial después de registrar cambio
